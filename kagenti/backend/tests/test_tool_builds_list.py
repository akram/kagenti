...
"""
Unit tests for the list tool builds endpoint (GET /tools/builds).

Tests cover:
- Response shape and only tool builds returned
...
from app.core.constants import KAGENTI_TYPE_LABEL, RESOURCE_TYPE_TOOL
from app.routers.tools import router
from app.services.kubernetes import get_kubernetes_service


def _make_build(name: str, namespace: str = "team1", git_url: str = "https://github.com/example/tool-repo", revision: str = "main") -> dict:
    """Minimal Shipwright Build resource for tools."""
    return {
        "metadata": {
            "name": name,
            "namespace": namespace,
...
def _make_buildrun(build_name: str, run_suffix: str, phase: str = "Succeeded", failure_message: str = None) -> dict:
    ...
    status = {
        ...
        "output": {"image": "registry.local/tool-img:tag", "digest": "sha256:def"},
    }
...
class TestListToolBuilds:
    """Tests for GET /api/v1/tools/builds."""
    ...
    def test_empty_namespace_returns_empty_list(self, client, mock_kube):
        ...
        try:
            with patch("app.core.auth.settings") as auth_settings:
                ...
                response = client.get("/api/v1/tools/builds?namespace=team1")
    ...
    def test_returns_builds_with_buildruns(self, client, mock_kube):
        ...
        builds = [_make_build("my-tool", namespace="team1")]
        buildruns = [
            _make_buildrun("my-tool", "xyz89", phase="Succeeded"),
        ]
        ...
        try:
            ...
            item = data["items"][0]
            assert item["buildName"] == "my-tool"
            assert item["namespace"] == "team1"
            assert item["buildRegistered"] is True
            assert item["gitUrl"] == "https://github.com/example/tool-repo"
            assert item["gitRevision"] == "main"
            assert item["toolName"] == "my-tool"
            assert len(item["buildRuns"]) == 1
            br = item["buildRuns"][0]
            assert br["name"] == "my-tool-run-xyz89"
    ...
    def test_build_with_no_buildruns(self, client, mock_kube):
        ...
        builds = [_make_build("orphan-tool-build")]
